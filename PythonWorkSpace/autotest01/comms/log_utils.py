...
def get_logger():
    ...
    sh2 = logging.FileHandler(filename=r'D:\PythonWorkSpace\autotest01\logs\info.log', mode='a', encoding='utf-8')
    sh2.setLevel('INFO')

    # 输出到./error.log 文件,并且内容追加写入,并且级别ERROR及ERROR级别以上的内容
    sh3 = logging.FileHandler(filename=r'D:\PythonWorkSpace\autotest01\logs\error.log', mode='a', encoding='utf-8')
    sh3.setLevel('ERROR')
