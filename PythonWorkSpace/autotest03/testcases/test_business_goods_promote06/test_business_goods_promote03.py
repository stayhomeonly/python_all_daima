...
class TestGoodsPromote(unittest.TestCase):
    cases = ReadExcel.read_data_all(DATA_FILE, "Business_goods_promote")
    ...
    def testgoodspromote(self, case):
        ...
        if "#time#" in case.case_data:
            time = (datetime.datetime.now() + datetime.timedelta(days=1)).strftime('%Y-%m-%d')  # 获取当前时间+1天
            if case.case_id == 10:  # 开始促销时间小于当前时间的场景
                time = (datetime.datetime.now() + datetime.timedelta(days=-1)).strftime('%Y-%m-%d')  # 获取当前时间+1天
            case.case_data = replace_data(case.case_data, "promoteStartTime", time)
        if "#goodsId#" in case.case_data:
            if case.case_id == 24:  # 商品已开启促销的场景
                sql = "select * from tb_goods where isPromote = 0 and isOnSale = 0 and goodsId != %s" \
                      " order by rand() limit 1;"
                one = self.db.find_one(sql, (get_ini_data('goodsId1', 'goodsId'),))
