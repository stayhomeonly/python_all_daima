...
class TestGoodsInfo(unittest.TestCase):
    cases = ReadExcel.read_data_all(DATA_FILE, 'Business_token_goodsInfo_id')
    ...
    def test_goods_info(self, case):
        ...
        try:
            ...
            ReadExcel.write_data(DATA_FILE, 'Business_token_goodsInfo_id', case.case_id, 7, '失败')
            logger.error('测试案例{},测试用例标题{},执行失败!实际结果{}'.format(case.case_id, case.case_title, res))
            logger.exception(e)
            raise e

        else:
            ReadExcel.write_data(DATA_FILE, 'Business_token_goodsInfo_id', case.case_id, 7, '成功')
