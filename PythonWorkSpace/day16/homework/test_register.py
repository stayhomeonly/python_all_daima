...
from day16.homework.excel_utils import ReadExcel
from day16.homework.log_utils import logger
from day16.homework.db_utils import DBUtils

cases = ReadExcel.read_data_all(r'cases.xlsx', 'register')
...
class TestRegister(unittest.TestCase):
    ...
    def test_register(self, case):
        ...
        try:
            ...
            ReadExcel.write_data('cases.xlsx', 'register', case.case_id, 7, '失败') # 7 几列的意思
            logger.error("测试编号｛｝，测试用例标题｛｝，执行失败!实际结果｛｝".format(case.case_id, case.case_title, res_body))
            logger.exception(e)
            raise e
        else:
            ReadExcel.write_data('cases.xlsx', 'register', case.case_id, 7, '成功')
            logger.info("测试编号｛｝，测试用例标题｛｝，执行成功!".format(case.case_id, case.case_title, res_body))
